...
function AppContent() {
  ...
  const { execute: registerDevice, loading } = useApi('registerDevice');
  ...
  const setupNotifications = async () => {
    try {
      ...
      Alert.alert('Notification Setup Failed', 'Unable to setup push notifications.');
    }
  };
  ...
  const handleRegisterDevice = async () => {
    ...
    if (result.success) {
      ...
      const errorMsg = result.error?.message || 'Failed to register';
